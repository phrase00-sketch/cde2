...
function buildPlayerBridge(dur, hasAudio, resumeT, bounds, resumePaused){
  ...
  var common =
    ...
    // Infer absolute time from rendered scene delays, independent of CSS variable
    // names or global overlay siblings. Explicit mode declarations take priority.
    'var cssStage=null;' +
    "function absoluteCssStage(){  var el=document.querySelector('[data-cde-stage][data-render-mode=\"css\"]');  if(!el)return null;  var mode=el.getAttribute('data-cde-time-mode');  if(mode==='scene-relative')return null;  if(mode==='absolute')return el;  try{    var b=JSON.parse(el.getAttribute('data-bounds'));    var cs=Array.from(el.querySelectorAll('[data-screen-label],section[id^=\"S_\"]')).filter(function(e){      for(var p=e.parentElement;p&&p!==el;p=p.parentElement)if(p.matches('[data-screen-label],section[id^=\"S_\"]'))return false;      return true;    });    if(!Array.isArray(b)||b.length<2||cs.length!==b.length||b[0]!==0)return null;    for(var i=0;i<b.length;i++){      if(!Number.isFinite(b[i])||(i&&b[i]<=b[i-1]))return null;      var st=getComputedStyle(cs[i]),names=st.animationName.split(','),delays=st.animationDelay.split(',');      if(!names.some(function(name,k){var d=delays[k%delays.length].trim(),n=parseFloat(d)/(d.endsWith('ms')?1000:1);return name.trim()!=='none'&&Number.isFinite(n)&&Math.abs(n-b[i])<.001;}))return null;    }    return el;  }catch(e){return null;}}function absoluteVideoStart(v,stage){  if(!stage||!stage.contains(v))return null;  /* The nearest animated wrapper may introduce a shot partway through a scene. */  for(var el=v;el&&el!==stage;el=el.parentElement){    var st=getComputedStyle(el),names=st.animationName.split(','),delays=st.animationDelay.split(','),starts=[];    names.forEach(function(name,k){var d=delays[k%delays.length].trim(),n=parseFloat(d)/(d.endsWith('ms')?1000:1);if(name.trim()!=='none'&&Number.isFinite(n))starts.push(n);});    if(starts.length)return Math.min.apply(Math,starts);  }  return 0;}" +
    'function applyTime(t){if(t<0)t=0;if(DUR>0&&t>=DUR)t=(DUR>0.02?DUR-0.02:0);if(typeof window.__cdeSetClock==="function")window.__cdeSetClock(t);if(!seekOmStage(t,playing)&&!seekExplicitDeck(t)){var s=deckSlider();if(s){var mx=parseFloat(s.max||"0")||0;var v=(DUR>0&&mx>0)?(t/DUR*mx):((mx>0&&t>mx)?mx:t);setSlider(s,v);}else{setDeckScene(t);var ms=Math.max(0,((cssStage=absoluteCssStage())?t:t-sceneStart(t))*1000),as=anims();for(var i=0;i<as.length;i++){try{as[i].pause();as[i].currentTime=ms;}catch(e){}}}}syncVids(t,playing);bgmSync(t,playing);}' +
    'function vT0(v,t){var a=parseFloat(v.getAttribute("data-t0"));if(isFinite(a))return a;var start=absoluteVideoStart(v,cssStage);return start===null?sceneStart(t):start;}' +
